import { cartAtom, cartItemCountAtom, cartTotalPriceAtom, clearCartAtom, removeFromCartAtom, updateCartItemQuantityAtom } from "@/atoms/cart.atom";
...
import { AlertCircle, ArrowRight, Minus, Plus, ShoppingBag, ShoppingCart, Trash2, X } from "lucide-react";
                    ...
                    <div className="flex flex-1 flex-col">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center border rounded-full overflow-hidden shadow-sm">
                          <button
                            className={`px-2 py-0.5 text-gray-700 dark:text-gray-300 ${item.quantity <= 1
                          ...
                          <button
                            className={`px-2 py-0.5 text-gray-700 dark:text-gray-300 ${item.quantity >= (item.medicine.variants.limitQuantity ?? Infinity)
                                ? 'opacity-50 cursor-not-allowed'
                                : 'hover:bg-green-50 dark:hover:bg-green-900/20'
                              }`}
                            onClick={() => {
                              const newQuantity = item.quantity + 1;
                              if (newQuantity <= (item.medicine.variants.limitQuantity ?? Infinity)) {
                            ...
                            disabled={item.quantity >= (item.medicine.variants.limitQuantity ?? Infinity)}
                          >
                            <Plus size={12} />
                          </button>
                        </div>
