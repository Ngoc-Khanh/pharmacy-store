...
import { AlertCircle, Filter, RotateCcw, Search, SlidersHorizontal, ChevronDown, UserCheck, UserCog, User2 } from "lucide-react";
...
export function UsersTableToolbar<TData>({
  ...
  return (
    <div className="bg-white dark:bg-slate-950 rounded-xl border border-cyan-100 dark:border-cyan-800/30 shadow-md p-5">
        ...
        <div className="flex flex-col space-y-2 md:space-y-0 md:flex-row md:items-center gap-3 w-full">
          ...
          >
            <div className="absolute left-0 top-0 bottom-0 w-10 flex items-center justify-center">
              <Search className="h-4 w-4 text-cyan-500 dark:text-cyan-400" />
            </div>
            <Input
              placeholder="Tìm kiếm người dùng..."
              value={table.getState().globalFilter}
              onChange={(event) => table.setGlobalFilter(event.target.value)}
              className="h-10 pl-10 w-full bg-white dark:bg-slate-950 border-cyan-200 dark:border-cyan-800/30 focus-visible:ring-cyan-500/30 focus-visible:border-cyan-400 dark:focus-visible:border-cyan-700 dark:placeholder:text-slate-500 rounded-md shadow-sm transition-all duration-200"
            />
          </motion.div>
          ...
          <div className="flex flex-col space-y-2 md:space-y-0 md:flex-row flex-wrap gap-2 w-full md:w-auto">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="h-10 border-cyan-200 dark:border-cyan-800/30 hover:bg-cyan-50 dark:hover:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400"
                >
            ...
            {(table.getColumn("status") || table.getColumn("role")) && (
              ...
              >
                <div className="hidden md:flex items-center px-3 py-1.5 rounded-md bg-gradient-to-r from-cyan-50 to-sky-50 dark:from-cyan-900/20 dark:to-sky-900/20 text-sm text-cyan-700 dark:text-cyan-400 font-medium border border-cyan-100 dark:border-cyan-800/30 shadow-sm">
      ...
      {!hasRows && isFiltered && (
        ...
        >
          ...
          <div>
            <p className="font-medium mb-0.5">Không tìm thấy người dùng nào phù hợp</p>
