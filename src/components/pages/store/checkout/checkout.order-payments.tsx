...
interface CheckoutOrderPaymentsProps {
  user: UserResponse;
...
export function CheckoutOrderPayments({ user, onAddressSelect, selectedAddress, showNewAddressForm, paymentMethod, onPaymentMethodChange }: CheckoutOrderPaymentsProps) {
  const navigate = useNavigate();

  // Set default address
  useEffect(() => {
    if (!user?.addresses?.length || selectedAddress) return;
    const defaultAddress = user.addresses.find((addr: UserAddress) => addr.isDefault);
    const addressToSelect = defaultAddress || user.addresses[0];
    onAddressSelect(addressToSelect.id);
