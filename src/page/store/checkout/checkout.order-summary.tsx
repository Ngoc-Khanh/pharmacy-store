...
import { clearCartAtom } from "@/atoms/cart.atom";
...
export function CheckoutOrderSummary({ cart, totalPrice, selectedAddress, showNewAddressForm, paymentMethod }: Props) {
  ...
  const { mutate: placeOrder, isPending: isProcessing } = useMutation({
    mutationFn: (orderData: PlaceOrderDto) => {
      return StoreAPI.PlaceOrder(orderData);
    },
    onSuccess: (data) => {
